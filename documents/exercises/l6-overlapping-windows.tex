...
Let \(\cZ\) be a finite or countable alphabet, let
\(p:\cZ\to[0,1]\) satisfy \(\sum_{z\in\cZ}p(z)=1\), and let
\((Z_t)_{t\geq1}\) be iid \(\cZ\)-valued random variables with law
\(p\). Let \(L\geq2\) be an integer, and define the \(\cZ^L\)-valued
...
\[
 \beta_W(a)=1-\left(\sum_{z\in\cZ}p(z)^2\right)^{L-a}.
\]
Explain from this formula why the coefficient is close to one when the
collision probability \(\sum_{z\in\cZ}p(z)^2\) is small or when the number \(L-a\)
