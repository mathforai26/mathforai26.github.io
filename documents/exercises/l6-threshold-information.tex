...
Let \(N\geq6\), let \(\cK=\{2,\ldots,N-1\}\), and, for
\(k\in\cK\), define \(f_k(x)=\I\{x\geq k\}\) on \([N]\).
...
\(\{(k-1,0),(k,1)\}\) identifies \(k\) uniquely within \(\cK\).
Explain why no single labeled example identifies an interior threshold
\(k\in\{3,\ldots,N-2\}\). Give one-example teaching sets for the two boundary
thresholds \(k=2\) and \(k=N-1\).
