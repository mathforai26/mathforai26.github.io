...
% Build identity: the compilation timestamp appears on every page.
...
material can be read separately.
...
a positive numerator with zero denominator contributes \(+\infty\).
...
\]
The minimum is attained; a constant cost has \(v_P(c)=0\).

\begin{remark}[Variance proxy vs. variance]
...
\section{How does sequence length affect KL control?}
...
\section{How should we fit a model for good task performance?}
\label{sec:better-control}

At a given likelihood-fitting accuracy, generated cost can still depend
strongly on how the model behaves after errors. We can improve this
behavior in two ways: evaluate consequences that distinguish the models,
or impose structure on error propagation. Direct task feedback and
further-ahead prediction will distinguish $Q_-$ and $Q_+$:
\(Q_-\) returns to zero after its random first bit, while \(Q_+\)
copies that bit forever (\cref{eq:endpoint-generators}).
A recovery constraint will improve \(Q_+\) without changing its
likelihood on the data.
We first make precise what fitting accuracy means, then develop these
two approaches and ask which information should persist through recovery.

\subsection{Likelihood fitting at finite accuracy}
\label{sec:likelihood-pitfall}

The natural unit for our optimization objective is loss per predicted
token. For \(n\) observed sequences, define empirical \emph{per-token}
log loss
\[
 \widehat L(Q)=\frac1{nT}\sum_{i=1}^n\sum_{t=1}^T
 -\log Q(X_t^{(i)}\mid X_{<t}^{(i)}).
\]
This averages the losses of the \(nT\) prediction tasks.
We consider this normalization despite the earlier warning
in \cref{sec:late-predictions},
because log-loss is conventionally reported per token.
We express a target optimization accuracy as
\begin{equation}
 \widehat L(\widehat Q)\le\min_{Q\in\mathcal Q}\widehat L(Q)+\eta,
 \qquad \eta\ge0.
 \label{eq:approximate-fit}
\end{equation}
The \emph{optimization tolerance} \(\eta\) bounds the remaining gap to
the best empirical fit in the class. Certifying such a gap requires
an optimization analysis; here we ask what achieving it buys us.
Multiplying by \(T\) gives the corresponding per-sequence tolerance
\(T\eta\). Maintaining a fixed per-sequence tolerance as \(T\)
increases would require increasingly accurate per-token optimization.

Increasing the number of examples creates a second reason for finer
accuracy: to retain a decreasing statistical error, the optimization
contribution should stay on that same scale.
More examples can supply more gradient updates within a single pass,
so a smaller tolerance may be reached without increasing the number
of passes over the data.

For all-zero data and a class containing \(P=\delta_{0^T}\), the
minimum empirical loss is zero. In this example the gap is simply the
observed loss, and its effect on final-answer accuracy is exact.
...
and \(1-e^{-\kappa}\). Thus approximation error can also leave a
...
Likelihood fitting still gives a positive statistical guarantee.
The following result states it directly in terms of generated cost,
with the optimization tolerance retained.
...
The proof is in \cref{app:finite-mle}.
...
\end{equation}
At fixed model class and confidence level, the first term decreases
as \(1/n\). \emph{Optimizing to statistical accuracy} means keeping
...
For a fixed target cost \(s\), it is enough to make
\(2\log(|\mathcal Q|/\delta)/n+T\eta\le s\).

A fixed tolerance leaves the term \(T\eta\) in the guarantee as
\(n\) grows.
\Cref{prop:endpoint-optimization} gives final-answer error
\(1-e^{-T\eta}\sim T\eta\) when \(T\eta\) is small. Thus the
optimization contribution has the correct leading order in this example,
even with infinitely many demonstrations and a fixed cost range.

Lecture~18 develops this comparison for imitation learning.
The comparison in \cref{sec:growing-horizon} fixes infinite-sequence
...
We now give additional objectives that distinguish generators within
the accuracy permitted by likelihood fitting.
...
Both matrices are column-stochastic. This convention matches column-vector
state probabilities; the earlier kernel notation is \(K(j,i)=A_{ij}\).
...
The allowed excess empirical loss above the best fit in the model class.
It is measured here per token: a tolerance \(\eta\) permits per-sequence
