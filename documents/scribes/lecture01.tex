...
rows of $P$ and $H^0$ store the transposes of position and hidden vectors.
...
\begin{remark}[Position information when $P=0$]
\label{rem:position-placement}
...
restriction, so $P=0$ alone does not make the whole decoder insensitive to order.
\end{remark}
...
\begin{remark}[Example: LayerNorm-type normalization]
\label{rem:rmsnorm}
...
\end{equation}
so computing the scores and weighted sums costs $\Theta(t^2d_h)$ arithmetic.
...
\begin{remark}[Observed long-context failures]
\label{rem:long-context-failures}
...
\input{../exercises/l1-rope.tex}
\input{../exercises/l1-greedy-decoding.tex}
\input{../exercises/l1-bounded-attention.tex}
