...
def main():
    ...
    lines = [r"\begin{table}[htbp]", r"\centering\small",
             ...
             r"Training \(T\) & Model & Excess MSE & KL/step & Unstable / 50\\\midrule"]
    for row in rows:
        name = "AR(32)" if row["method"]=="ar" else "Rank-one state"
        lines.append(f'{row["n"]} & {name} & {row["excess_mse"]:.4f} & '
                     f'{row["conditional_kl"]:.4f} & {row["unstable"]}' + r"\\")
    lines += [r"\bottomrule\end{tabular}",
              ...
              r"KL uses each fitted residual variance. Unstable means generative",
